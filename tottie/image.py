"""General image utilities for 64x64 LED matrix displays."""
...
def crop_and_resize(
    ...
    """Crop or letterbox to sizexsize.

    fit controls how the image is fitted to the square target:
      "cover"   — crop to a square at `anchor`, then resize to sizexsize (default).
                  Fills the target; pixels outside the crop are discarded.
      "contain" — letterbox: scale to fit entirely within sizexsize preserving
    ...
    return img.crop((left, top, left + m, top + m)).resize((size, size), Image.Resampling.LANCZOS)
